...
class S3toS3Interaction(object):
    ...
    """
    def __init__(self, source_aws_key: str,
    ...
    def duplicate_objects(self, 
        ...
        self.__do_transfer(source_bucket = source_bucket,
            ...
            if payload['KeyCount'] == 0:
                ...
                for item in payload['Contents']:
                    ...
                    if filepath.endswith(source_bucket_suffix):
                        ...
                        if filename is not '':
                            logging.info(f'Sending file {source_bucket}/{filepath} to {target_bucket}/{target_path}/{filename}')
                            logging.info(f'filename to {operation}: {filename}')
                            copy_source = {
                                'Bucket': source_bucket,
                                'Key': filepath
                            }
                            if not self.is_test:
                                ...
                                if copy_response['ResponseMetadata']['HTTPStatusCode'] != 200:
                                    logging.error(f"Failed to {operation}: {fileName}")
